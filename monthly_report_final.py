...
def _parse_args():
    ...
    parser.add_argument("--input-dir", help="월별 입력 폴더. 기본값은 기존 data/YYYY-MM/input입니다.")
    parser.add_argument("--output-dir", help="완성본 저장 폴더. 기본값은 기존 data/YYYY-MM/output입니다.")
...
INPUT_DIR = os.path.abspath(ARGS.input_dir) if ARGS.input_dir else os.path.join(MONTH_DIR, "input")
CUSTOMER_DIR = os.path.join(INPUT_DIR, "customers")
OUTPUT_DIR = os.path.abspath(ARGS.output_dir) if ARGS.output_dir else os.path.join(MONTH_DIR, "output")
...
OUTPUT_FILE    = os.path.join(
    OUTPUT_DIR,
    f"사업계획대비매출실적분석 ({YEAR_SHORT}년 {MONTH}월)_완성.xlsx",
)
# =============================================
# 공통 유틸
# =============================================
